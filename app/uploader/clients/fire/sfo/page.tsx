...
export default function SfoUploader() {
    ...
    Papa.parse(file, {
      ...
      complete: (result) => {
        ...
        if (
          ...
          setWarning(
            `Expected headers: ${expectedHeaders}, Expected number of columns: ${expectedHeaders.length}`
          );
          setTableData([]);
          return;
        }
    ...
    <div className="grid grid-rows-[10px_1fr_10px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <main className="flex flex-col gap-[32px] row-start-2 items-center">
        <Image
          ...
          priority
        />
        <h1 className="text-4xl">Import SFO's from CSV file</h1>
        ...
        <button
          ...
          className="cursor-pointer mt-2 px-4 py-2 bg-[#031D64] text-white rounded-md hover:bg-blue-700 transition"
        >
          Process file here
        </button>
